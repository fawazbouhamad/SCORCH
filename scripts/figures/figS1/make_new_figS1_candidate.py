#!/usr/bin/env python3
# Manuscript Figure S.1 composite (four panels). Since the v1.0.0
# pre-release correction the composite is FULLY REGENERATED from deposited
# data and is byte-identical to the canonical manuscript Figure S.1
# (sha256 050a0721...); the superseded hybrid embed (a5632323...) is
# historical only. This compositor merges the upstream REGENERATED panels
# and does not alter their scientific content; the ACTIVE panels (c)/(d)
# are regenerated by shipped producers, and only the LEGACY frozen
# fallback donor lacks its original runnable producer.
#
...
#   bottom row : the REGENERATED GHCNd-vs-ERA5 station strip (panels c, d),
#                produced from the deposited series by
#                make_figS1_station_panels.py + make_figS1_station_strip.py.
#                The SUPERSEDED frozen donor at
#                assets/frozen_figures/figS1_station_donor/
#                Figure_S2_station_provisional.png is a clearly-labelled
#                HISTORICAL FALLBACK only; that legacy drawing (not the
#                active panels) has no original runnable producer
#                (transcribed provenance). The active metrics are
...
# v1.0.0 pre-release correction: panels (c)/(d) are now REGENERATED from the deposited GHCNd and ERA5
# series by make_figS1_station_panels.py + make_figS1_station_strip.py. Prefer
# that strip; fall back to the frozen provisional donor only if it is absent,
# in which case Figure S.1 remains a hybrid composite and must be described as
# one. STATION_STRIP has no leading "(a)/(b)" label band -- the compositor adds
# all four canonical letters itself -- so the legacy crop is skipped for it.
STATION_STRIP = os.path.join(OUT_DIR, "Figure_S1_station_panels_strip.png")
FROZEN_DONOR_B = os.path.join(RELEASE_ROOT, "assets", "frozen_figures",
                              "figS1_station_donor",
                              "Figure_S2_station_provisional.png")
DONOR_B_IS_REGENERATED = os.path.exists(STATION_STRIP)
DONOR_B = STATION_STRIP if DONOR_B_IS_REGENERATED else FROZEN_DONOR_B
print("[NEW-S1] station panels (c)/(d): "
      + ("REGENERATED from the deposit" if DONOR_B_IS_REGENERATED
         else "FROZEN donor -- Figure S.1 is a HYBRID COMPOSITE"))
for donor, hint in ((DONOR_A, "run make_figS1_type3_event25.py first"),
                    (DONOR_B, "run make_figS1_station_strip.py first")):
...
def main():
    ...
    # -- donor B body --------------------------------------------------------
    # The FROZEN provisional donor carries its own "(a)"/"(b)" label strip above
    # the plots, which must be cropped before this figure adds the canonical
    # (c)/(d) letters. The REGENERATED strip has no such band, so cropping it
    # would silently remove real plot content.
    if DONOR_B_IS_REGENERATED:
        B_body = B
        print(f"[NEW-S1] donor B is the regenerated station strip "
              f"({B.shape[1]}x{B.shape[0]} px); no label-strip crop applied")
    else:
        bb = ink_row_blocks(B)
        assert len(bb) >= 2, bb
