...
import sys
# Phase 2C candidate copy: outputs into the candidate workspace; the baked-in
# "(a)"/"(b)" labels of the AB figure are SUPPRESSED (the Figure 3 assembler
# now controls all five panel labels) and the AB sub-panel bounding boxes are
# exported to a JSON sidecar so the assembler can place them mathematically.
...
def plot_panel_a_axes(ax, df):
    ...
    # Phase 2C: baked-in "(a)" suppressed — assembler controls all labels.
...
def plot_panel_b_ellipse_frequency(ax, df, fig):
    ...
    # Phase 2C: baked-in "(b)" suppressed — assembler controls all labels.
...
def make_figure_ab(df):
    ...
    # Phase 2C: export the exact sub-panel bounding boxes as fractions of the
    # SAVED (tight-bbox, pad=0.1in) image so the assembler can position the
    # (a)/(b) labels mathematically.
    import json
    fig.canvas.draw()
    _rend = fig.canvas.get_renderer()
    _tb = fig.get_tightbbox(_rend)
    _pad = 0.1
    _W = _tb.width + 2 * _pad
    _H = _tb.height + 2 * _pad

    def _frac(a):
        bb = a.get_window_extent(_rend).transformed(
            fig.dpi_scale_trans.inverted())
        return dict(x0=(bb.x0 - _tb.x0 + _pad) / _W,
                    x1=(bb.x1 - _tb.x0 + _pad) / _W,
                    y0=(bb.y0 - _tb.y0 + _pad) / _H,
                    y1=(bb.y1 - _tb.y0 + _pad) / _H)

    with open(os.path.join(OUT_DIR, "fig03_ab_panel_bboxes.json"), "w",
              encoding="utf-8") as _fh:
        json.dump({"a": _frac(fig.axes[0]), "b": _frac(fig.axes[1])},
                  _fh, indent=1)
...
def make_individual_panels(df):
    ...
    save_single_panel_with_colorbar(
        ...
        # Phase 3F LABEL-ONLY revision: the counted quantity is ellipse
        # footprints per cell (freq[inside] += 1 per ellipse), not events.
        "PCA-ellipse footprints per grid cell"
    )
