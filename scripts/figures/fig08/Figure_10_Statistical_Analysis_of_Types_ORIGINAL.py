...
# (original committed script had OUT_DIR = "ENTER_OUTPUT_DIRECTORY" and read a
# phantom "SHWE_spatial_PCA_metrics_blobfirst_clean_p60_merge_revert.csv").
import sys
# Phase 2C candidate copy: outputs into the candidate workspace; (a)/(b)/(c)
# row labels re-placed canonically (computed from the row bounding boxes).
...
OUT_DIR = os.path.join(_SCORCH_REPRO, "fig08")
os.makedirs(OUT_DIR, exist_ok=True)

METRICS_CSV = MASTER_CSV
...
def prepare_dataframe(metrics_csv):
    ...
    # Clean-package input repair: event id and typology come straight from the
    # master dataset columns (new_event_id, type, event_type_original — the v3
    # reclassified typing that matches Table 1 and the reference figure), NOT
    # from the stale date-range catalogue above, which pre-dates the v3
    # reclassification (it splits Type 1/2 as 4/3 events instead of 3/4) and
    # mis-assigns ellipses when event windows overlap. The catalogue functions
    # are retained above for reference but are no longer called.
...
def main():
    ...
    # Phase 2C canonical row labels: horizontal, Arial bold 17 pt black,
    # ONE shared left offset, each mathematically centered on the vertical
    # span of its logical row (rows found by clustering axes bounding boxes
    # at the two largest vertical gaps) — replaces the manual fig.text
    # placements (0.870/0.550/0.210) that left "(a)" visibly shifted.
    _boxes = [a.get_position() for a in fig.axes]
    _ycs = sorted((((b.y0 + b.y1) / 2.0) for b in _boxes), reverse=True)
    _gaps = sorted(range(len(_ycs) - 1),
                   key=lambda i: _ycs[i] - _ycs[i + 1], reverse=True)[:2]
    _cuts = sorted(((_ycs[i] + _ycs[i + 1]) / 2.0 for i in _gaps),
                   reverse=True)
    _rows = [[], [], []]
    for b in _boxes:
        yc = (b.y0 + b.y1) / 2.0
        _rows[0 if yc > _cuts[0] else (1 if yc > _cuts[1] else 2)].append(b)
    _x_left = min(b.x0 for b in _boxes) - 0.052   # clear of y-axis titles
    for _letter, _row in zip(("(a)", "(b)", "(c)"), _rows):
        _yc = (min(b.y0 for b in _row) + max(b.y1 for b in _row)) / 2.0
        fig.text(_x_left, _yc, _letter, ha="right", va="center",
