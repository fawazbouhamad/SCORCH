...
# =================== USER CONFIG ===================
DATADIR        = r"ENTER_PATH_TO_FOLDER_WITH_DAILY_NETCDF_FILES"  # folder with *_daily.nc files
FILE_GLOB      = os.path.join(DATADIR, "*_daily.nc")               

OUTDIR         = os.path.join(DATADIR, "p95_exceed_heatwaves_1deg_flat")
...
# Region of interest
LAT_MIN, LAT_MAX = 10.0, 46.0    # °N
LON_MIN, LON_MAX = 20.0, 70.0    # °E

# 1° × 1° box aggregation: "mean" (stable) or "max" (hotspotty)
BOX_AGG = "mean"
...
def to_boxes_dataframe(field2d, lat_name, lon_name, date_val):
    """
    Flatten a 2D field to rows and assign 1°×1° box centers (.5).
    ...
    # Assign to 1° cells centered at .5
...
def main():
    ...
    # 1) Sweep all files -> collect daily 1° box values into one long DataFrame
    for fp in files:
        ds = open_nc_safe(fp)
