...
* **Figure 4** is produced by
  `scripts/figures/fig04/correct_fig04_type4_horizontal.py`, which
  applies the smallest deterministic localized correction to the
  archived original slide export
  ...
  the two RIGHT Type 4 arrows are reversed (180-degree in-place pixel
  rotation of each arrow's own ink, with circle pixels protected) so
  the Type 4 progression reads horizontally left to right - two
  structures -> one structure -> two structures - with the first arrow
  pair converging on the middle circle and the second pair diverging
  to the right pair. Every pixel outside those two arrow boxes is
  unchanged; the script emits a difference mask and locality report
  proving it. (The earlier lower-arrow correction
  `correct_fig04_type4_arrows.py`, PNG `b747c8c8...`, produced a
  vertical temporal reading and is superseded; kept for history.)
...
| `fig04/Figure_04.png` | `c35d9ed6ccea8d7f6d8ec8ad92d65fb3c5dd2df16d82b015b7a531b2eefc829f` | yes (word/media/image4.png) |
| `fig04/Figure_04.pdf` | `6621d6048473e910d6b150dfe2d20709c82066ee82ce77659e4e55ff88cbe591` | (companion from the same producer, deterministic metadata) |
